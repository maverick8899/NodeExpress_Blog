...
const Account = require('../model/Account_ReactBase');
...
class Task {
    CheckRole(req, res, next) {
        ...
        role === 'user' || role === 'manager' || role === 'admin' ? next() : res.json('NOT PERMISSION');
    }
...
class Manager {
    CheckRole(req, res, next) {
        const role = req.data.role;
        role === 'manager' || role === 'admin' ? next() : res.json('NOT PERMISSION');
    }
    welcome(req, res) {
        res.json('Manager');
    }
